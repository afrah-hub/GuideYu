...
import { Mail, Lock, User, UserPlus, ArrowLeft, Apple, Compass, Brain, Eye, EyeOff } from 'lucide-react';
...
const Register = () => {
  ...
  const { register, googleLogin, login, user } = useAuth();
  ...
  const getStrengthColor = (s) => {
    if (password.length === 0) return 'bg-transparent';
    if (s <= 2) return 'bg-red-500 shadow-[0_0_10px_rgba(239,68,68,0.8)]';
    if (s <= 4) return 'bg-orange-500 shadow-[0_0_10px_rgba(249,115,22,0.8)]';
  ...
  return (
    <div className="min-h-screen mesh-gradient-theme flex font-poppins relative overflow-hidden">
      ...
      <div className="w-full flex h-full min-h-screen">
        ...
        <div className="flex-[1.2] flex flex-col items-center justify-center p-4 sm:p-8 pt-16 sm:pt-8 relative z-10 w-full my-auto">

          <motion.div
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ duration: 0.6 }}
            className="w-full max-w-[460px] bg-[var(--bg-surface)]  backdrop-blur-2xl border border-[var(--border-default)]  p-8 rounded-[40px] relative overflow-hidden shadow-2xl transition-all duration-500 hover:shadow-sm"
          >
            ...
            <form className="space-y-4 relative z-10" onSubmit={handleSubmit}>
              <div className="space-y-4">
                ...
                <div className="relative group">
                  ...
                  <label htmlFor="email-register" className="absolute left-12 top-2 text-[10px] font-bold text-[var(--text-tertiary)]  uppercase tracking-widest transition-all duration-300 peer-placeholder-shown:top-1/2 peer-placeholder-shown:-translate-y-1/2 peer-placeholder-shown:text-sm peer-placeholder-shown:font-medium peer-placeholder-shown:normal-case peer-focus:top-2 peer-focus:-translate-y-0 peer-focus:text-[10px] peer-focus:font-bold peer-focus:uppercase peer-focus:tracking-widest peer-focus:text-[var(--text-accent)]  pointer-events-none">
                    Email Address
                  </label>
                </div>
                ...
                <div className="relative group">
                  ...
                  <label htmlFor="password-register" className="absolute left-12 top-2 text-[10px] font-bold text-[var(--text-tertiary)]  uppercase tracking-widest transition-all duration-300 peer-placeholder-shown:top-1/2 peer-placeholder-shown:-translate-y-1/2 peer-placeholder-shown:text-sm peer-placeholder-shown:font-medium peer-placeholder-shown:normal-case peer-focus:top-2 peer-focus:-translate-y-0 peer-focus:text-[10px] peer-focus:font-bold peer-focus:uppercase peer-focus:tracking-widest peer-focus:text-[var(--text-accent)]  pointer-events-none">
                    Password
                  </label>
                  <button
                    type="button"
              ...
              {/* Terms */}
              <div className="flex items-start gap-3 px-2 pt-1 mb-1">
                <input type="checkbox" id="terms" required className="mt-0.5 w-4 h-4 rounded border-[var(--bg-surface)]/20 bg-black/20 text-[var(--text-accent)] focus:ring-blue-900 focus:ring-offset-0 focus:ring-offset-transparent cursor-pointer" />
                <label htmlFor="terms" className="text-[10px] text-[var(--text-tertiary)]  font-medium leading-relaxed max-w-[90%] transition-colors duration-700">
                  Join the professional registry and agree to <a href="#" className="text-[var(--text-accent)]  font-bold hover:underline underline-offset-4">Terms & Conditions</a>.
                </label>
              </div>
              ...
              <button
                ...
                className={`w-full relative group overflow-hidden px-8 py-4 bg-[var(--bg-base)]  rounded-2xl text-[var(--text-primary)]  font-bold text-sm uppercase tracking-widest shadow-2xl shadow-sm transition-all duration-500 hover:-translate-y-1 hover:shadow-sm mt-1 ${isSubmitting ? 'opacity-70 pointer-events-none' : ''}`}
              >
                <span className="relative z-10 flex items-center justify-center gap-3">
                  {isSubmitting ? 'Initializing...' : 'Create Account'} {!isSubmitting && <UserPlus size={20} className="group-hover:scale-110 transition-transform duration-300" />}
                </span>
            ...
            <p className="text-center mt-6 text-[var(--text-tertiary)]  text-[10px] font-bold uppercase tracking-widest relative z-10">
              Already registered? <Link to="/login" className="text-[var(--text-accent)]  font-black hover:underline underline-offset-4 ml-1">Sign In</Link>
            </p>
          </motion.div>
        </div>
      </div>
